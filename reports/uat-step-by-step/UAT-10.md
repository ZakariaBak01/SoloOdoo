...
4. U8: click **Confirm** or **Post** on the invoice (visible standard Odoo label). Click **Register Payment**, enter a partial payment of `2000` MAD using the UAT Bank journal, and confirm. Record the residual and payment status from the posted invoice or **Customers -> Invoices**. If the optional Enterprise `account_reports` module is installed, also inspect **Reporting -> Aged Receivable**.
