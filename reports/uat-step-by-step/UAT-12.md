...
1. U8: open **Invoicing → Customers → Invoices**. On three disposable invoices set due dates relative to today's UAT date, post them, and confirm residual > 0. In Company A's **Overdue Invoice Reminder Templates**, verify J+7/J+15/J+30 templates are configured.
...
6. U8: open **Invoicing → Vendors → Payments → New** (or the Payments menu in this Odoo layout). Create a posted outbound payment using cheque method; enter cheque number `UAT-CHQ-001`, click **Save → Confirm**, then **Print Cheque**. Record print timestamp and user.
